...
public class ProgressBar {
  ...
  public ProgressBar(Shell shell) {
    this.shell = shell;
  }
  ...
  public void display(int percentage, String statusMessage) {
    if (!started) {
      started = true;
      shell.getTerminal().writer().println();
    }
    int x = (percentage / 5);
    int y = 20 - x;
    String message = ((statusMessage == null) ? "" : statusMessage);

    String done = shell.getSuccessMessage(new String(new char[x]).replace("\0", doneMarker));
    ...
    shell.getTerminal().writer().println(CUU + "\r" + DL + progressBar + "% " + message);
